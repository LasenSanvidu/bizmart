...
// mount checking
  ...
  bool _validateInputs() {
    ...
    if (firstNameController.text.trim().isEmpty) {
      _showValidationError("First name cannot be empty");
      return false;
    }

    if (lastNameController.text.trim().isEmpty) {
      _showValidationError("Last name cannot be empty");
      return false;
    }
    ...
    // Check for valid mobile number format (basic validation)
    if (mobileController.text.isNotEmpty) {
      // Simple regex for checking if it's a valid number format
      RegExp mobileRegex = RegExp(r'^[0-9]{10}$');
      if (!mobileRegex.hasMatch(mobileController.text.trim())) {
        _showValidationError("Mobile number must be exactly 10 digits");
        return false;
      }
    }

    // Check username format (no spaces, special characters, etc.)
    RegExp usernameRegex = RegExp(r'^[a-zA-Z0-9_]+$');
    if (!usernameRegex.hasMatch(usernameController.text.trim())) {
    ...
    if (firstNameController.text == originalFirstName &&
        ...
        mobileController.text == originalMobile &&
        usernameController.text == originalUsername) {
      _showValidationError("No changes detected");
      return false;
    }
